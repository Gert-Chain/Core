...
export class QueryClientImpl implements Query {
  ...
  Params(request: QueryParamsRequest): Promise<QueryParamsResponse> {
    ...
    const promise = this.rpc.request("gert.scheduler.Query", "Params", data);
    return promise.then((data) => QueryParamsResponse.decode(new Reader(data)));
  }

  Hook(request: QueryGetHookRequest): Promise<QueryGetHookResponse> {
    const data = QueryGetHookRequest.encode(request).finish();
    const promise = this.rpc.request("gert.scheduler.Query", "Hook", data);
  ...
  HookAll(request: QueryAllHookRequest): Promise<QueryAllHookResponse> {
    ...
    const promise = this.rpc.request("gert.scheduler.Query", "HookAll", data);
    return promise.then((data) =>
